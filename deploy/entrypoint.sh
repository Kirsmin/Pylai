...
def remap(section: str, key: str, dest_name: str) -> None:
    ...
    if not raw: return
    src = Path(raw)
    if not src.is_absolute(): src = source.parent / src
    if not src.exists(): raise SystemExit(f"配置引用的敏感文件不存在: {src}")
...
# Independent DataProtection KEK. Operators may inject a read-only file by setting
# PYLAI_DP_KEK_FILE; otherwise the single-container deployment creates one in the data volume.
DP_KEK_FILE="$SECRET_DIR/dp-kek"
if [ -n "${PYLAI_DP_KEK_FILE:-}" ]; then
    if [ ! -s "$PYLAI_DP_KEK_FILE" ]; then
        echo "[pylai] DataProtection KEK 注入文件不存在或为空: $PYLAI_DP_KEK_FILE" >&2
        exit 1
    fi
    # Copy the injected key into our protected volume before dropping privileges.
    umask 077
    cp "$PYLAI_DP_KEK_FILE" "$DP_KEK_FILE"
elif [ ! -s "$DP_KEK_FILE" ]; then
    # Never manufacture a replacement key for a ring that is already encrypted with ours.
    if grep -Rqs '<pylai-aes-gcm' /var/lib/pylai/dataprotection 2>/dev/null; then
        echo "[pylai] 已存在 AES-GCM 加密的 DataProtection 密钥环，但持久化 KEK 丢失；拒绝生成新 KEK。" >&2
        exit 1
    fi
    # Legacy plaintext rings can start safely: new keys will be encrypted with this KEK.
    # Existing plaintext XML must be explicitly retired after its protected payloads expire.
    umask 077
    openssl rand 32 > "$DP_KEK_FILE"
fi
chmod 600 "$DP_KEK_FILE"
PYLAI_DP_KEK_FILE="$DP_KEK_FILE"
export PYLAI_DP_KEK_FILE

chown -R pylai:pylai /var/lib/pylai

run_as_pylai() {
    local command
    command=$(printf '%q ' env PYLAI_DP_KEK_FILE="$PYLAI_DP_KEK_FILE" "$@")
...
if [ -f "$PG_HBA" ]; then
    if ! grep -q "^host all all 127.0.0.1/32" "$PG_HBA"; then echo "host all all 127.0.0.1/32 scram-sha-256" >> "$PG_HBA"; fi
    if ! grep -q "^host all all ::1/128" "$PG_HBA"; then echo "host all all ::1/128 scram-sha-256" >> "$PG_HBA"; fi
else
...
for i in $(seq 1 30); do su postgres -c "$PG_BIN/pg_isready -q -h /run/postgresql -p 5432" && break; sleep 1; done
...
for i in $(seq 1 20); do redis-cli -p 6379 -a "$PYLAI_REDIS_PASSWORD" --no-auth-warning ping >/dev/null 2>&1 && break; sleep 0.2; done
cd /opt/pylai
...
echo "[pylai] 数据库、签名密钥与 DataProtection KEK 检查完成，启动服务..."
