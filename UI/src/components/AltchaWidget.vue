...
function onStateChange(ev: Event) {
  ...
  if (state === 'verified') {
    ...
    if (payload) { commitPayload(payload); emit('verified', payload) }
...
async function ensureVerified(): Promise<AltchaVerificationResult> {
  ...
  try { result = await element.verify() } catch { result = null }
...
watch(enabled, (value) => { if (!value) commitPayload(null) })

onMounted(async () => {
  try { enabled.value = (await loadPublicConfig()).altchaEnabled } catch { enabled.value = true }
  if (enabled.value) await fetchChallenge()
})
